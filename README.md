...
- **`turn-service.input-token-budget-per-turn`** — optional, defaults
...
- **`turn-service.project-file-cache-bytes`** — optional, defaults to
...
| Live analytics | `GET /api/chat/signals` (last computed signal values, active project), `GET /api/core/projects/{project_id}/metrics` (metrics_framework, computed on demand), `POST /api/triggers/preview` |
