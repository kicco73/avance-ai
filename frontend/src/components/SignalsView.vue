...
// Drives the bars' brief flash animation (see .signal-bar-changed) so a
// live update reads as "this one moved" instead of silently jumping —
// shared with EditProjectView.vue's Inspector Signals tab.
const { recentlyChanged, markChanged } = useSignalChangeFlash()

async function fetchSignalsAndTriggers() {
  let signalsOk = true
  try {
    const nextSignals = await getSignals()
    markChanged(signals.value, nextSignals)
