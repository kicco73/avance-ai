...
// The opening message (regenerated by reset/switch/upload/delete) can fail
// on the backend without failing the operation itself — the response just
// carries an extra `opening_message_error` field when that happened. Feeds
// the same shared error area as every other error, no separate UI.
function surfaceOpeningMessageError(result) {
  if (result?.opening_message_error) {
    setApiError(result.opening_message_error.message, result.opening_message_error.detail)
  }
}
...
async function handleReset() {
  ...
  try {
    ...
    state.value = null
    handleStateChange(newState)
    surfaceOpeningMessageError(newState)
...
async function handleModelUploadChange(event) {
  ...
  try {
    ...
    surfaceOpeningMessageError(result)
