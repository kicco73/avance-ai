...
async def test_seeds_the_default_project_when_the_database_has_none(app_db):
    project_service, controller, job_queue, broadcaster = _build(app_db)
    assert app_db.list_projects() == []

    await _seed_default_project_if_empty(app_db, project_service, controller, job_queue, broadcaster)
