...
def test_download_backup_returns_a_sqlite_file(client):
    response = client.get("/api/core/settings/backup")
...
def test_restore_a_valid_backup_succeeds(client):
    backup = client.get("/api/core/settings/backup").content

    response = client.post(
        "/api/core/settings/backup", content=backup, headers={"Content-Type": "application/octet-stream"}
    )
...
def test_restore_rejects_a_schema_mismatch(client, tmp_path):
    ...
    response = client.post(
        "/api/core/settings/backup", content=wrong, headers={"Content-Type": "application/octet-stream"}
    )
...
def test_app_keeps_working_after_a_rejected_restore(client, tmp_path):
    wrong = _make_sqlite_bytes(tmp_path, "wrong.db", ["CREATE TABLE unrelated (id INTEGER PRIMARY KEY)"])
    client.post("/api/core/settings/backup", content=wrong, headers={"Content-Type": "application/octet-stream"})
