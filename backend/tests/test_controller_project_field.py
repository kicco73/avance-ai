...
class TestPutProjectField:
    ...
    def test_rejects_an_invalid_taken_or_empty_id_and_a_field_off_the_whitelist(self, client, hello_project):
        ...
        assert client.put(f"/api/projects/{hello_project}/project/id", json={"value": "not a valid id"}).status_code == 400
        assert client.put(f"/api/projects/{hello_project}/project/id", json={"value": "taken"}).status_code == 400
        assert client.put(f"/api/projects/{hello_project}/project/id", json={"value": ""}).status_code == 400
        assert client.put(f"/api/projects/{hello_project}/project/name", json={"value": "x"}).status_code == 400
    ...
    def test_unknown_project_is_404_on_get_and_put(self, client):
        assert client.get("/api/projects/does-not-exist/project").status_code == 404
        assert client.put("/api/projects/does-not-exist/project/ui-label", json={"value": "x"}).status_code == 404
