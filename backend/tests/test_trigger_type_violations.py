...
def test_flags_regardless_of_operand_order():
    assert len(trigger_type_violations("5 <= system.today()")) == 1


def test_flags_only_the_offending_leg_of_a_chained_comparison():
    violations = trigger_type_violations("0 <= signal.mood < system.time()")
    assert len(violations) == 1
...
def test_two_string_typed_identifiers_may_be_compared():
    ...
    assert trigger_type_violations("system.today() >= system.time()") == []


def test_equality_is_never_checked_regardless_of_type():
    # == and != never raise in Python, whatever the two types are.
    assert trigger_type_violations("system.today() == 5") == []
    assert trigger_type_violations("system.today() != signal.mood") == []
...
def test_no_violation_when_either_side_is_not_statically_typeable():
    ...
    assert trigger_type_violations("env.visits >= 5") == []
    assert trigger_type_violations("system.today() >= env.threshold") == []


def test_no_violation_for_two_unresolvable_operands():
    assert trigger_type_violations("env.a >= env.b") == []

def test_session_metric_and_metric_namespaces_are_numeric():
    assert trigger_type_violations("session.metric.engagement() >= 50") == []
    assert trigger_type_violations("metric.retention() >= 50") == []


def test_flags_a_known_string_session_field_compared_numerically():
    violations = trigger_type_violations("session.last_user_session_datetime >= 5")
    assert len(violations) == 1


def test_numeric_session_fields_are_not_flagged():
    assert trigger_type_violations("session.current_session_duration_in_minutes >= 5") == []
    assert trigger_type_violations("session.number_of_user_sessions >= 5") == []
    assert trigger_type_violations("session.state_duration_in_minutes >= 5") == []


def test_combines_multiple_violations_in_one_expression():
    violations = trigger_type_violations("system.today() >= 5 and system.time() <= 10")
