...
def test_env_is_scoped_per_project_too(db):
    ...
    session_id = _session(db, project_name="proj-a")
    db.set_env(session_id, {"a": "1"})
...
def test_reset_project_wipes_env_for_every_user_of_that_project(db):
    ...
    other_session = _session(db, username="alice", project_name="other-proj")
    db.set_env(alice_session, {"a": "1"})
    db.set_env(bob_session, {"a": "1"})
    db.set_env(other_session, {"a": "1"})
...
def test_set_env_writes_onto_the_given_session_never_the_latest_live_one(db):
    ...
    row = Tracking.get(Tracking.env.is_null(False))
