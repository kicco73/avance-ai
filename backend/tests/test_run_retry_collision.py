...
def test_create_test_replaces_a_dead_row_at_the_same_cache_key(db: Db):
    """A prior run with no results and no live job (TestCache's
    own "dead attempt" case, see find()'s docstring) must not block a
    retry at the exact same (session, strategy, edit_count,
    labeling_revision) cache key — create_test's own unique
    ...
    first = db.create_test(None, "proj", session_id, "batch", 1, 0, {})
    assert first["results"] is None

    second = db.create_test(None, "proj", session_id, "batch", 1, 0, {})

    assert second["results"] is None
    assert db.list_tests("proj", session_id) == [second]
