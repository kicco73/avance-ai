...
def _save_index_yml(db, state_key: str) -> None:
    db.save_project_files(
        PROJECT_ID, {"index.yml": _index_yml(state_key).encode("utf-8")}, {"index.yml": "text/yaml"}
    )
...
def _save_two_state_index_yml(db) -> None:
    db.save_project_files(
        PROJECT_ID, {"index.yml": TWO_STATE_YML.encode("utf-8")}, {"index.yml": "text/yaml"}
    )
...
def test_load_project_at_revision_caches_by_project_and_revision(db, project_service):
    ...
    rev0 = project_service._automaton_loader.load_at_revision(PROJECT_ID, 0)
    rev1 = project_service._automaton_loader.load_at_revision(PROJECT_ID, 1)
    ...
    # other one.
    assert project_service._automaton_loader.load_at_revision(PROJECT_ID, 0) is rev0
    assert project_service._automaton_loader.load_at_revision(PROJECT_ID, 1) is rev1
...
def test_get_draft_automaton_and_state_works_for_a_never_published_project(db, project_service):
    ...
    automaton, state = project_service.get_automaton_and_state(PROJECT_ID, type='test')
...
def test_get_draft_automaton_and_state_sees_the_in_progress_draft(db, project_service):
    ...
    automaton, _ = project_service.get_automaton_and_state(PROJECT_ID, type='test')
...
def test_get_automaton_and_state_for_session_uses_that_sessions_own_pinned_revision(db, project_service):
    ...
    old_session_id = db.create_chat_session(
        username=Session().user, project_id=PROJECT_ID,
        revision=db.get_project_published_revision(PROJECT_ID), start_state="a",
    )
    ...
    new_session_id = db.create_chat_session(
        username=Session().user, project_id=PROJECT_ID,
        revision=db.get_project_published_revision(PROJECT_ID), start_state="b",
    )
...
def test_get_automaton_and_state_for_session_works_for_a_draft_session_too(db, project_service):
    ...
    draft_session_id = db.create_chat_session(
        username=Session().user, project_id=PROJECT_ID,
        revision=db.get_project_revision(PROJECT_ID), start_state="a", type="test",
    )
