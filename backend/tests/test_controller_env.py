...
delete_env_key/clear_env (chat.env.Env) — {"stored": ..., "computed": ...}
split so the "Edit project" view's Inspector Env tab knows which values
are actually editable/deletable. `message_id`, when given, restricts to
values as they stood at or before that exact message — same point-in-time
convention as GET /api/chat/metrics.
"""
