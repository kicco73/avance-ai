"""A state's own `ai-may-read-sources:`/`ai-must-read-sources:` fields
(State.ai_may_read_sources/ai_must_read_sources) — the subset of the
...
time against `sources:`, the same way action.target is validated
against declared states — plus each named source's own required
`ai-definition`. The legacy names (`tools`, `ai-may-query-sources`,
`ai-must-query-sources`) are rejected outright here, as is the removed
`ai-may-write-sources` — sources are read-only, a state can no longer
declare one the model may write to.
"""
...
def test_each_field_is_parsed_into_its_own_tuple_leaving_the_other_empty():
...
def test_ai_may_write_sources_is_rejected_outright_sources_are_read_only():
...
@pytest.mark.parametrize("field_name", ["ai-may-read-sources", "ai-must-read-sources"])
...
def test_a_source_may_not_be_both_may_and_must_read():
