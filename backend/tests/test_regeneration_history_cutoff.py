...
def _automaton() -> Automaton:
    ...
    state_a = State(key="a", ui_label="A", final=False, contextual_prompt="hi", actions=[advance], history_cutoff=False)
    state_b = State(key="b", ui_label="B", final=True, contextual_prompt="there", history_cutoff=True)
