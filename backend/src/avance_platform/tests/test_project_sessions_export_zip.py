...
def _download_zip(client, project_id: str) -> dict[str, bytes]:
    response = client.get(f"/api/skills/platform/projects/{project_id}")
...
def test_sessions_json_never_appears_among_the_projects_own_files(client, hello_project):
    client.post(f"/api/skills/platform/projects/{hello_project}/sessions/import", files=[("files", ("t.txt", "user: hi\nassistant: yo\n", "text/plain"))])
...
def test_download_then_reupload_round_trips_the_imported_session(client):
    ...
    assert resp.status_code == 200, resp.text
    resp = client.post(
        f"/api/skills/platform/projects/{project_id}/sessions/import", files=[("files", ("t.txt", "user: hi\nassistant: yo\n", "text/plain"))]
    )
    assert resp.status_code == 200, resp.text
    ...
    resp = client.post(
        "/api/skills/platform/projects/upload", content=zip_bytes, headers={"Content-Type": "application/zip"}
    )
    assert resp.status_code == 200, resp.text
