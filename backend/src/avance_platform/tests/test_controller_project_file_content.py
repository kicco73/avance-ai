...
def _put_css(client, project_id: str, css: bytes):
    return client.put(f"/api/skills/platform/projects/{project_id}/files/index.css", content=css)


def _publish_two_state_project_with_red_css(client) -> None:
    client.post("/api/skills/platform/projects/upload", content=TWO_STATE_YML.encode(), headers={"Content-Type": "application/x-yaml"})
...
def test_get_project_file_reports_content_type_and_real_byte_size_with_content_only_for_text_files(client, hello_project):
    ...
    response = client.get(f"/api/skills/platform/projects/{hello_project}/files/index.css")
...
def test_a_missing_index_css_is_204_no_content_while_any_other_missing_file_is_404(client, hello_project):
    ...
    response = client.post("/api/skills/platform/projects/upload", content=yml.encode(), headers={"Content-Type": "application/x-yaml"})
    assert response.status_code == 200, response.text
    project_id = parse_sse_result(response)["project_id"]

    response = client.get(f"/api/skills/platform/projects/{project_id}/files/index.css")
    assert response.status_code == 204
    assert response.content == b""

    assert client.get(f"/api/skills/platform/projects/{hello_project}/files/does-not-exist.txt").status_code == 404
