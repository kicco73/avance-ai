...
| `.env-<key>-<value>` | A class added to `.chat-window-shell` for every env key the project declares `ui-binding: true` | `<value>` is the key's current value, following every later change of it; any character outside `A-Z a-z 0-9 _ -` becomes `_` (`very sad` → `env-mood-very_sad`, `2.5` → `env-level-2_5`). No class while the value is empty or null. It is there from the moment the window enters the conversation, after a reload too, and a window showing another conversation shows that one's values. E.g. `.env-mood-sad .chat-header { ... }`. |
