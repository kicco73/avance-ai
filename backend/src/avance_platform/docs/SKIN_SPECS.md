...
| `.env-<key>-<value>` | A class added to `.chat-window-shell` for every env key an on-exit bound with `chat.bind_env(env.<key>)` | `<value>` is the key's current value, following every later change of it; any character outside `A-Z a-z 0-9 _ -` becomes `_` (`very sad` → `env-mood-very_sad`, `2.5` → `env-level-2_5`). No class while the value is empty or null. Gone after `chat.unbind_env_all()` and when the window moves to another conversation, e.g. `.env-mood-sad .chat-header { ... }`. |
