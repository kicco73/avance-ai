...
| `.action-buttons` / `.action-btn` | The row of manual-action buttons (when the current state has any) | Colors only — `background`, `border`, `color`, `:hover`, `:disabled`. Layout (padding, wrapping, sizing) is fixed and not meant to be overridden. A `:hover` rule, anywhere in the skin, is applied only on devices that can hover: touch browsers keep `:hover` on the last tapped element until the next tap, so on a phone the rule would paint the button as pressed after the tap is over. |
...
| `.env-<key>-<value>` | A class added to `.chat-window-shell` for every env key an on-exit bound with `chat.bind_env(env.<key>)` | `<value>` is the key's current value, following every later change of it; any character outside `A-Z a-z 0-9 _ -` becomes `_` (`very sad` → `env-mood-very_sad`, `2.5` → `env-level-2_5`). No class while the value is empty or null. The binding belongs to the conversation, not the window: it is still there after a reload, and a window showing another conversation shows that one's bindings. A key written in the same on-exit right after `chat.bind_env` shows its new value. Gone after `chat.unbind_env_all()`, e.g. `.env-mood-sad .chat-header { ... }`. |
