"""What every screen needs and no single one owns: the boot state, the
reference docs, AI model selection, and the per-project views the
Inspector and the editor read.

Named for the platform rather than for the chat because that is what is
left here: the chat window's own routes moved out with the service, into
webchat/webchat_controller.py. Nothing in this file is specific to a
browser chat, and a build without the native interface still answers all
of it — WhatsApp needs the legal terms and the project views as much as
the SPA does.
"""
