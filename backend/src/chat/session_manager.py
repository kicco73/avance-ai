"""Owns the chat-session lifecycle (open/closed, creation, rotation) on
top of db.py's pure ChatSession CRUD — no business rules live in db.py
itself. See db.py's ChatSession for why the model isn't called `Session`
(that name is session.py's unrelated process-local singleton).

Constraint enforced here (not just assumed): for a given `username` +
`project_name` there is at most one *writable* session at a time — the
one with the most recent `datetime_start`. The chat is driven by a
single per-project state automaton, so letting two concurrently-open
sessions both write would race on that automaton's state; older sessions
that are still technically "open" (by the 1-hour idle rule) are simply
frozen for writes once a newer one exists, whether that newer one was
created automatically (the old one went idle) or manually (see
create_session, e.g. an explicit "new session" UI action).
"""
...
class ChatSessionManager(object):
    ...
    def get_or_create_current_session(
        ...
        one (which immediately becomes "the" current session by having
        ...
        if latest is not None and self.is_open(latest, now):
            if session_id is not None and session_id != latest["id"]:
                logger.info(
                    "get_or_create_current_session(): caller's session_id=%s is stale for %s/%s, "
                    "current session is %s", session_id, username, project_name, latest["id"]
                )
