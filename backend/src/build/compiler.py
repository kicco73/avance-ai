...
off disk when something actually needs one rather than inlined.
...
# Where this product's own project files live. Nothing is loaded here:
# AUTOMATON.archives_dir below hands this to the platform's own reader,
# which opens a file when a turn actually asks for one.
...
def compile_module(automaton: Any, storage_revision: int | None = None) -> tuple[str, str]:
    ...
    # automaton holds.
    parts.append(f"_GENERAL_ATTACHMENTS = {_literal(automaton.general_attachments)}")
...
# is handed (see tracking.project_files), and an automaton that carries
# its own files — archives_dir, set to data/ just below the AUTOMATON
# literal — gets the reader that opens them off disk. The same goes for
# attachment.read and for a turn's own attachments.
