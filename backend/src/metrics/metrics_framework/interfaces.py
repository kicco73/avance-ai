from __future__ import annotations

from datetime import datetime
from typing import Protocol

from .dto import MetricResult, UserAnalyticsData
from .scope import MetricScope


class AnalyticsDb(Protocol):
    def list_chat_sessions(self, username: str, project_id: str) -> list[dict[str, object]]: ...
    def get_chat_session(self, session_id: int) -> dict[str, object] | None: ...
    def get_messages(self, session_id: int, last_n: int | None = None, since: datetime | None = None) -> list[dict[str, object]]: ...
    def get_signals(self, session_id: int) -> list[dict[str, object]]: ...


class MetricCalculator(Protocol):
    @property
    def name(self) -> str: ...

    @property
    def ui_label(self) -> str: ...

    @property
    def ui_description(self) -> str: ...

    @property
    def scope(self) -> frozenset[MetricScope]: ...

    def calculate(self, data: UserAnalyticsData) -> MetricResult: ...
