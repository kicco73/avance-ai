...
| `session.ended` | server → client | `{reason}` — closed, by the person or by the server itself (`channel-switch`, `force-new-session`, `revision-invalid`, `final-state`). `final-state` is a turn that moved the conversation into a state with no outgoing action: it has nowhere left to go, so `turn/input_listener.py` closes the session once every frame of that turn is out. A conversation that *starts* in such a state (a one-state project) is not closed by it — only arriving there is. Published from the one place every closure passes through, `SessionManager.close_session`. It is also the last thing said about that session: whoever was watching it stops, whoever held it forgets it |
