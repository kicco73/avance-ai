...
| `env.memory_changed` | `{key, value}` — one memory note the model wrote and that was actually merged into the turn's memory scope (`ai/turn/tracking_processor.py`'s `GlobalMemoryScope`/`LocalMemoryScope.merge`), one message per key — same reason `env.changed` is one per key. A `none`-scope state, or a `local`-scope merge the scope itself discarded, writes nothing and publishes nothing |
