...
| `session.info` | server → client | `{state, services, audio, current, channel, project_id, reply_silence_seconds}` — which conversation this is and everything describing it. `reply_silence_seconds` is how long a client may hear nothing during an exchange before it treats the reply as lost (`turn-service.reply-silence-seconds`); the server's own deadlines on the model are shorter, so an `output.error` normally arrives first. `current` says the session is the one its type's active slot holds, **never that you may write to it**: writability is that AND `channel` being your own (see "Whose conversation it is") |
...
| `state.signals` | `{values}` — the signal values this exchange ran on, `{name: value}`, said only **when it had any**: a reader keeps the last set it was told about. A turn that measured nothing publishes nothing. A manually fired action measures nothing of its own and carries the session's last recorded set forward (`turn/input_processor.py`), so it says the same values again rather than going silent. The values are also recorded on the exchange's own tracking row, which is what a reader refetches to place them in the session's history (`GET /api/core/sessions/{id}/signals`) |
| `env.changed` | `{key, value}` — one env key an action wrote, one message per key: whoever cares that a key moved does not care how many others moved with it. Same envelope as `state.changed`. Never pushed by default: it reaches only an identity's connections that registered for it (`CLIENT_REGISTRABLE`), never the per-session "who is watching" path `state.changed` uses |
| `env.memory_changed` | `{key, value}` — one memory note the model wrote and that was actually merged into the turn's memory scope (`ai/turn/tracking_processor.py`'s `GlobalMemoryScope`/`LocalMemoryScope.merge`), one message per key — same reason `env.changed` is one per key. A `none`-scope state, or a `local`-scope merge the scope itself discarded, writes nothing and publishes nothing. Delivered like `env.changed`: only to connections that registered for it |
| `env.bound` | `{key, value}` — an on-exit's `chat.bind_env(env.<key>)` (`tracking/actuators/chat_namespace.py`): from now on that session's chat window carries the key's value. `value` is what the key held at the call. Never reaches a client as such: `system/bus_channel.py` records the binding for the session and sends `env.bindings` (see "Websocket frames") |
| `env.unbound` | `{}` — an on-exit's `chat.unbind_env_all()`: nothing is bound in that session any more. Same path as `env.bound` |
| `state.buttons` | `{actions}` — what can be done now, and the **only** place the choices are: no state payload carries them. The pressable actions first, then — for every `list` env key a trigger of the state reads — one entry per current option, named `choice:<key>:<index>` with the option as its `ui_button`/`ui_label` and `target` `""`; pressing one sends that `name` back as `input.button` unchanged. When the key's options are profiles (see PROJECT_SPECS.md, "List keys"), each entry also carries `profile: {title, description, key}`, with `picture_url` too when the option had one — `title` and `description` translated like a label, `key` never — and its `ui_button`/`ui_label` is the title; the `name` sent back is the same `choice:<key>:<index>` |
...
| `output.drive` | `{path}` — `None` for a bulk clear | `tracking/actuators/drive_namespace.py`, after every `drive.write(...)` (`path` set to the written path); `project/project_controller.py`'s `POST .../drive/save-media` and `POST .../drive/download-media` (the PDF preview dialog's Save/Download buttons, `role="customer"`), after adding one of a project's own `media/` files to the caller's own drive; its `DELETE .../drive/{path}` (the Drive tab's delete button, `role="customer"`), after removing a file from the caller's own drive; and `turn/sessions/session_manager.py`'s `clear_drive_of_type`, when a new test session clears a previous test session's drive (`path: None` — the receiving end already treats any `output.drive` as "refetch the listing", so no per-path detail is needed here) — `output.*` for what it names, delivered like the rest of this table (an identity's registered connections, envelope `project_id`/`session_id`), never through the per-session "who is watching" path `output.text`/`state.changed` use |
...
state.signals                   only if the turn had signal values
...
| `env.bindings` | server → browser | `{values}` — every env key bound in that session (`env.bound`) with its current value, the whole set each time. Sent to the connections watching the session when a key is bound or unbound, and whenever `env.changed` passes on the Bus for a bound key; and to a connection as it starts watching. Needs no registration, and never carries a key that is not bound. The bindings live in the socket's process, per session: they survive a reload or a reconnection, not a restart, and go with `session.ended` |
...
`session.taken_over`, `ui.progress`, `output.drive`, `env.changed`,
`env.memory_changed`) + `human_prompt`.
