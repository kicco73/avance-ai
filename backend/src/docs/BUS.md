...
| `env.bindings` | `{values}` — the current value of every env key the session's project declares `ui-binding: true` (see PROJECT_SPECS.md, env), said once as a conversation is entered, right after `session.info` (`turn/input_listener.py`), and only when the project declares any. What follows is the regular `env.changed` of those keys: nothing re-sends the set. A chat channel forwards it to the connection that entered (`webchat/webchat_service.py`) |
...
| `env.bindings` | server → browser | `{values}` — the Bus's `env.bindings`, answering a `session.enter`: every `ui-binding` key of that session with its current value. Needs no registration. The browser keeps those keys and follows them through `env.changed`, which it registers for |
