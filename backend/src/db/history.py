...
class HistoryMixin:
    ...
    def _pop_history(self, user_id: str, project_id: str, archive_name: str, kind: str) -> bytes | None:
    ...
    def undo_project_file(self, user_id: str, project_id: str, archive_name: str, current_content: bytes) -> bytes | None:
        previous = self._pop_history(user_id, project_id, archive_name, 'undo')
        if previous is None:
            return None
        self._push_history(user_id, project_id, archive_name, 'redo', current_content)
        return previous

    def redo_project_file(self, user_id: str, project_id: str, archive_name: str, current_content: bytes) -> bytes | None:
        next_content = self._pop_history(user_id, project_id, archive_name, 'redo')
