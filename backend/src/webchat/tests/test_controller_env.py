"""GET/PUT/DELETE /api/skills/platform/sessions/{session_id}/env — returns
...
def test_put_env_value_stores_it(client, hello_project):
    ...
    assert client.get(f"/api/skills/platform/sessions/{session_id}/env").json()["memory"] == {"favorite_color": "blue"}
...
def test_delete_env_value_removes_the_key(client, hello_project):
    ...
    response = client.delete(f"/api/skills/platform/sessions/{session_id}/env/favorite_color")

    assert response.status_code == 200
    assert response.json()["memory"] == {"mood": "happy"}
    assert client.get(f"/api/skills/platform/sessions/{session_id}/env").json()["memory"] == {"mood": "happy"}
...
def test_env_with_a_message_id_restricts_to_a_point_in_time(client, hello_project):
    ...
    live = client.get(f"/api/skills/platform/sessions/{session_id}/env").json()
    as_of_message = client.get(f"/api/skills/platform/sessions/{session_id}/env?message_id={message_id}").json()
