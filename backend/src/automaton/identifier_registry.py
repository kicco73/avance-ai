...
class IdentifierRegistry:
    ...
    CHAT: dict[str, str] = {
        ...
        "write_table": "Same as chat.write, but the body is a markdown table built from `table`: a dict {column name: [cells]} (cells are strings, numbers or booleans), one column per key in order — exactly what source.<name>.select_subtable(...) returns, e.g. chat.write_table(source.casos.select_subtable('caso', 'titulo')) or chat.write_table({'Name': ['Alice', 'Bob'], 'Score': [7.5, 4]}). An empty dict writes nothing. Only available in the on-exit script of an action whose target state declares input-processor: system.",
    }
    REPLY_METHODS: tuple[str, ...] = ("write", "write_table")
