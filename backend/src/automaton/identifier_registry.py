...
class IdentifierRegistry:
    ...
    TASK: dict[str, str] = {
        ...
        "websearch": "Searches the web and returns what it found as CSV — e.g. task.websearch('well-reviewed dentists in Barcelona'). The pages a search engine returns are read, a model decides which columns describe them and fills them in, and the CSV text comes back (header row first) for a later statement in the same task to use. Nothing is written to the project: this is the same search the Source card's own AI Web Import runs, with its result returned instead of stored.",
