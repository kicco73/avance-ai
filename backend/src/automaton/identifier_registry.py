...
class IdentifierRegistry:
    ...
    CHAT: dict[str, str] = {
        ...
        "bind_env": "Binds one env key to the chat window's look — e.g. chat.bind_env(env.mood). From then on the chat window carries the CSS class env-<key>-<value> (env-mood-happy), following every later change of that key, so a skin can style the window by its value. The argument must be env.<key> itself, and a list key is not accepted. Several calls bind several keys. Only available in an action's own on-exit script, and only takes effect in webchat.",
        "unbind_env_all": "Drops every binding chat.bind_env made — e.g. chat.unbind_env_all(). The env-<key>-<value> classes leave the chat window. Only available in an action's own on-exit script, and only takes effect in webchat.",
        "write": "Writes body_md as a message of the automaton's own — e.g. chat.write('Step %d of %d' % (env.step + 1, len(env.questions))). Towards an input-processor: system state it is that state's reply; towards an input-processor: ai state it is a message of its own, next to the model's reply: before it when the action fired before the model answered, after it otherwise. Several calls join as paragraphs; the text is saved in the transcript like any reply. Only available in an action's own on-exit script.",
        "write_table": "Same as chat.write, but the body is a markdown table built from `table`: a dict {column name: [cells]} (cells are strings, numbers or booleans), one column per key in order — exactly what source.<name>.select_subtable(...) returns, e.g. chat.write_table(source.casos.select_subtable('caso', 'titulo')) or chat.write_table({'Name': ['Alice', 'Bob'], 'Score': [7.5, 4]}). An empty dict writes nothing. Only available in an action's own on-exit script.",
    }
