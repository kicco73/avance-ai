...
from ai.turn.attachments import load_attachments
from tracking.env import PersistedEnv
from tracking.evaluation_scope import EvaluationScopeBuilder
from tracking.fixed_project_context import FixedProjectContext
from tracking.project_files import PROJECT_FILE_CACHE, project_files_for
from tracking.session_facts import SessionFacts
from ai.turn.tracking_processor import _turn_attachment_paths, estimate_state_prompt
...
def _automaton(revision: int) -> Automaton:
    ...
    state_a = State(
        input_processor="ai", key="a", ui_label="A", final=False, contextual_prompt="You are in A.", actions=[advance],
...
def _processor(db, automaton: Automaton, session_id: int, ai_service: RecordingSchemaAiService) -> TrackingProcessorAfterUserMessage:
    ...
    return TrackingProcessorAfterUserMessage(ai_service, scope_builder, env, TurnTransaction(db, session_id, []), user_variables)
