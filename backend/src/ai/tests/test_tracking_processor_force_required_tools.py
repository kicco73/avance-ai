"""TrackingProcessor.force_required_tools_for — whether ai-must-read-sources
...
def _processor(db, session_id: int, state: State) -> TrackingProcessor:
    ...
    processor.transaction = TurnTransaction(db, session_id, [])
    processor.user = user
    return processor
...
def test_a_state_with_no_ai_must_read_sources_is_never_forced(db):
    session_id = _session(db)
    state = State(input_processor="ai", key="a", ui_label="A", final=False, contextual_prompt="hi")
...
def test_the_first_turn_after_entering_the_state_is_forced(db):
    session_id = _session(db)
    state = State(input_processor="ai", key="a", ui_label="A", final=False, contextual_prompt="hi", ai_must_read_sources=("flights",))
