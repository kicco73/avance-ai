...
from ai.turn.env_prompt_block import ENV_BLOCK_HEADER, EnvPromptBlock
from ai.turn.turn_size_estimate import estimate_turn_request
...
STATE_A = State(input_processor="ai", key="a", ui_label="A", final=True, contextual_prompt="hi", input=("flight", "customer_email"))
STATE_B = State(input_processor="ai", key="b", ui_label="B", final=True, contextual_prompt="hi", input=("flight", "customer_email"))
STATE_C = State(input_processor="ai", key="c", ui_label="C", final=True, contextual_prompt="hi")
STATE_D_OUTPUT_ONLY = State(
    input_processor="ai", key="d", ui_label="D", final=True, contextual_prompt="hi", input=("flight",), output=("confidence",)
)
...
def _automaton(*states: State) -> Automaton:
    ...
    all_states = {"": State(input_processor="ai", key="", ui_label="", final=False, actions=[init_action])}
...
def test_a_dict_or_list_value_renders_as_json_not_a_python_repr():
    state = State(input_processor="ai", key="d", ui_label="D", final=True, contextual_prompt="hi", input=("_flight_record",))
