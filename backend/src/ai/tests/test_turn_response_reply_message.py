...
def _automaton() -> Automaton:
    state_a = State(input_processor="ai", key="a", ui_label="A", final=False, contextual_prompt="You are in A.")
...
async def test_reply_carries_the_persisted_assistant_message(db):
    ...
    processor = TrackingProcessorAfterUserMessage(PlainAiService(), scope_builder, env, TurnTransaction(db, session_id, []), user_variables)

    result = await processor.process("hi")

    assert result["assistant_message_id"].id is not None
    assert len(result["reply"]) == 1
    reply_message = result["reply"][0]
    assert reply_message["id"] == result["assistant_message_id"].id
