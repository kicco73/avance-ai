...
def _automaton(written_keys: dict[str, str] | None = None, memory_scope: str = "none") -> Automaton:
    ...
    state_b = State(
        input_processor="ai", key="b", ui_label="B", final=True, contextual_prompt="You are in B.",
        input=input_names, ai_memory_scope=memory_scope,
    )
...
async def test_regeneration_call_does_not_request_signals(db):
    ...
    processor = TrackingProcessorAfterUserMessage(
        ai_service, scope_builder, env, TurnTransaction(db, session_id, []), _user_variables(automaton, session_id)
    )
