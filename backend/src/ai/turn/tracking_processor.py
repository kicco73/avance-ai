...
from tracking.actuators import MutedReply, ReplySink
...
from tracking.sources import SourceNamespace, ToolSet
...
class TrackingProcessor(object):
	...
	def reply_after_answer(self, state: State, on_metadata: MetadataCallback) -> AsyncIterator[str]:
	...
	async def regenerate_reply(self, state: State, on_metadata: MetadataCallback) -> AsyncIterator[str]:
		...
		async for chunk in self.assistant_talker.chat(
			...
			tool_set=self.build_tool_set(state),
