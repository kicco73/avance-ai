"""Integration tests for POST /api/skills/testing/projects/{project_name}/aggregations/users,
...
def test_users_aggregation_averages_value_across_users(client, app_db, hello_project):
    ...
    response = client.post(
        f"/api/skills/testing/projects/{hello_project}/aggregations/users", json={"strategy": "turn_by_turn"},
    )
...
def test_users_aggregation_with_a_single_user_matches_that_users_own_run(client, app_db, hello_project):
    ...
    own_response = client.post(
        f"/api/skills/testing/projects/{hello_project}/runs/users/alice", json={"strategy": "turn_by_turn"},
    )
...
def test_users_aggregation_rerun_skips_dependency_resolution_when_cached(client, app_db, hello_project):
    ...
    with patch.object(UsersAggregationJob, "_resolve_or_construct_dependencies", spy):
        ...
        second = client.post(f"/api/skills/testing/projects/{hello_project}/aggregations/users", json={"strategy": "turn_by_turn"})
...
def test_users_aggregation_rejects_unknown_strategy(client, hello_project):
    response = client.post(
        f"/api/skills/testing/projects/{hello_project}/aggregations/users", json={"strategy": "nonsense"},
    )
